...
UPDATE `recipes`
SET `name` = 'Beetroot & onion seed soup'
WHERE recipe_id = 2;
...
INSERT INTO `recipeimages` (`image_id`, `image_name_path`) VALUES
(7, './assets/images/sweet-potato-coconut-curry.jpg');
...
(57, 'Buck wheat'),
(58, 'Cashew nuts'),
(59, 'Desiccated coconut'),
(60, 'Kaffir lime leaves'),
(61, 'Green chillies');
...
DELETE FROM `recipeingredients` WHERE `recipe_id` = 16;

SELECT recipeingredients.recipe_id as recipe_id, recipeingredients.ingredient_id as ingredient_id, ingredients.name as ingredient, recipeingredients.quantity as quantity, 
            recipeingredients.comments as comments
            FROM ((recipeingredients
            JOIN recipes ON recipeingredients.recipe_id=recipes.recipe_id)
            JOIN ingredients ON recipeingredients.ingredient_id=ingredients.ingredient_id)
            WHERE recipeingredients.recipe_id = 17;
